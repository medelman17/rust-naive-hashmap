    ...
    V: ::std::fmt::Debug,
{
    ...
    /**
     * First, key is hashed. Second, Linear search performed through entire data store
     * to find an index in said store where one of two conditions hold:
     * (1) New hash is greater than some hash in the store, insert key/value pair;
     * (2) New has is equal to some hash in the store; replace existing value.
     */
